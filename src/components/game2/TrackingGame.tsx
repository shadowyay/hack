  ...
  const AnalysisScreen = () => {
    ...
    return (
      <div className="absolute inset-0 bg-black/80 flex items-center justify-center z-20">
        <div className="bg-amber-900 border-4 border-amber-600 rounded-lg p-8 max-w-2xl mx-4 text-white">
          <h2 className="text-3xl font-bold text-center mb-6 text-amber-200">🏆 HUNT ANALYSIS</h2>
          
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-4">
              <h3 className="text-xl font-semibold text-amber-300">Performance Stats</h3>
              <div className="space-y-2">
                <div>Total Time: {totalTime} seconds</div>
                <div>Animals Killed: {gameState.killedAnimals}</div>
                <div>Final Score: {gameState.score}</div>
                <div>Accuracy: {accuracyPercentage}% ({accurateKills}/{gameState.killStats.length})</div>
                <div>Avg Kill Time: {averageKillTime}s</div>
                <div>Animals Tracked: {gameState.trackedAnimals}</div>
              </div>
            </div>
            
            <div className="space-y-4">
              <h3 className="text-xl font-semibold text-amber-300">Kill Breakdown</h3>
              <div className="space-y-2">
                {Object.entries(animalCounts).map(([animal, count]) => (
                  <div key={animal} className="flex justify-between">
                    <span>{animal}:</span>
          ...
          <div className="mt-6 text-center">
            <div className="mb-4">
              <h3 className="text-lg font-semibold text-amber-300 mb-2">Performance Rating</h3>
              <div className="text-2xl">
                {accuracyPercentage >= 80 ? '🥇 Legendary Hunter' :
                 accuracyPercentage >= 60 ? '🥈 Expert Tracker' :
                 accuracyPercentage >= 40 ? '🥉 Skilled Hunter' :
                 '🎯 Novice Hunter'}
              </div>
            </div>
