...
const RDR2TrackingGame: React.FC = () => {
  ...
  // UI state
  const [crosshairPos, setCrosshairPos] = useState<{ x: number; y: number }>({ x: 0, y: 0 });
  const [eagleEyeStatus, setEagleEyeStatus] = useState<string>('Ready');
  ...
  // Utility functions (must be above usage)
  ...
  const drawAnimal = useCallback((ctx: CanvasRenderingContext2D, animal: Animal, eagleEyeActive: boolean) => {
    ...
    if (animal.currentHealth < animal.maxHealth) {
      ...
      ctx.fillRect(animal.x - barWidth/2, animal.y - animal.size/2 - 10, (animal.currentHealth / animal.maxHealth) * barWidth, barHeight);
    }
  ...
  const shoot = useCallback((): void => {
    setGameState(prevState => {
      ...
      for (const animal of newState.animals) {
        ...
        if (animalDist < animal.size / 2 + 20) {
          ...
          if (newHealth <= 0) {
            ...
            continue; // animal is removed
          }
          updatedAnimals.push({ ...animal, currentHealth: newHealth });
        } else {
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent<HTMLCanvasElement, MouseEvent>): void => {
    mouseRef.current.clicked = true;
  }, []);

  const handleMouseUp = useCallback((e: React.MouseEvent<HTMLCanvasElement, MouseEvent>): void => {
    mouseRef.current.clicked = false;
  }, []);
